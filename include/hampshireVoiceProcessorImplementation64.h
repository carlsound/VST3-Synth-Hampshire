...
class VoiceProcessorImplementation64 : public Carlsound::Vst::VoiceProcessor
{
public:
		...
		if (VOICEPROCESSOR_BLOCKSIZE <= 0)	// voice processing happens in chunks of the block size
		{
			if (mClearOutputNeeded)
				for (Steinberg::int32 i = 0; i < kNumChannels; i++)
					memset(data.outputs[0].channelBuffers32[i], 0, data.numSamples * sizeof(double));
				...
				for (Steinberg::int32 i = 0; i < numEvents; i++)
				{
					if (inputEvents->getEvent(i, e) == Steinberg::kResultTrue)
					{
						...
						case Steinberg::Vst::Event::kNoteOnEvent:
						{
							...
							Carlsound::Hampshire::Voice<double>* voice = getVoice(e.noteOn.noteId);
							if (voice)
							{
						...
						case Steinberg::Vst::Event::kNoteOffEvent:
						{
							...
							Carlsound::Hampshire::Voice<double>* voice = findVoice(e.noteOff.noteId);
							if (voice)
							{
						...
						{
							Carlsound::Hampshire::Voice<double>* voice = findVoice(e.noteExpressionValue.noteId);
							if (voice)
							{
			...
			for (Steinberg::int32 i = 0; i < MAX_VOICES; i++)
			{
				...
				{
					if (!voices[i].process((double**)data.outputs[0].channelBuffers32, data.numSamples))
					{
						voices[i].reset();
						this->activeVoices--;
					}
...
protected:
	Carlsound::Hampshire::Voice<double>* getVoice(Steinberg::int32 noteId)
	{
		Carlsound::Hampshire::Voice<double>* firstFreeVoice = 0;
		if (noteId != -1)
		{
			for (Steinberg::int32 i = 0; i < MAX_VOICES; i++)
			{
	...
	Carlsound::Hampshire::Voice<double>* findVoice(Steinberg::int32 noteId)
	{
		if (noteId != -1)
		{
			for (Steinberg::int32 i = 0; i < MAX_VOICES; i++)
	...
	Carlsound::Hampshire::Voice<double> voices [MAX_VOICES];
};
